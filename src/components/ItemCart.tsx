...
export type Action = | { type: 'ADD', payload: { name: string, quantity: number } } | { type: 'REMOVE', payload: { name: string, quantity: number } }
    ...
    return (
        <li className="bg-black/40 p-3 rounded-lg flex flex-col gap-2 lg:flex-row">
            ...
            <div className="flex flex-row gap-2 lg:w-[30%]">
                ...
                <button className="rounded-md bg-medium text-white px-2 py-1 flex-1 max-w-[120px]">Eliminar</button>
            </div>
        </li>
    )
}